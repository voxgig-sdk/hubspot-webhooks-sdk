// Command HubspotWebhooks-mcp is an MCP server exposing the HubspotWebhooks
// SDK as MCP tools.
//
// Two tools are registered:
//
//	HubspotWebhooks_list  — list records of an entity
//	HubspotWebhooks_load  — load a single record of an entity
//
...
func main() {
	...
	// Configure from the environment: HUBSPOT_WEBHOOKS_APIKEY carries the API key and
	// HUBSPOT_WEBHOOKS_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("HUBSPOT_WEBHOOKS_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("HUBSPOT_WEBHOOKS_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewHubspotWebhooksSDK(opts)
