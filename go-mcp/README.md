# hubspot-webhooks-mcp

[MCP](https://modelcontextprotocol.io) server exposing the HubspotWebhooks SDK as
two agent tools — `hubspot-webhooks_list` and `hubspot-webhooks_load` — built on the
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hubspot-webhooks-mcp)
make build
...
claude mcp add --scope user hubspot-webhooks \
  -- /absolute/path/to/hubspot-webhooks-mcp -transport stdio

# 3b. …or run a shared HTTP server instead
./hubspot-webhooks-mcp -transport http -addr :8080
```
...
1. **Build** the server from this `go-mcp/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hubspot-webhooks-mcp
   ```
...
3. **Install it into Claude Code** (stdio transport):
   ...
   ```sh
   claude mcp add --scope user hubspot-webhooks \
     -- "$PWD"/dist/*/hubspot-webhooks-mcp -transport stdio
   ```
...
### Call the `hubspot-webhooks_list` tool
...
| Tool | Args | Returns |
|------|------|---------|
| `hubspot-webhooks_list` | `entity` (required), `query` (optional map) | First page of records as JSON |
| `hubspot-webhooks_load` | `entity` (required), `query` = `{id:N}` | Single record as JSON |
...
```sh
./hubspot-webhooks-mcp -transport http -addr :18080 &
...
curl -sN -X POST http://localhost:18080 \
  ...
  -d '{"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"hubspot-webhooks_load","arguments":{"entity":"basic","query":{"id":1}}}}'
```
