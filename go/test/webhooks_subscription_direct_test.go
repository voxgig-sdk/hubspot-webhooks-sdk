...
func TestWebhooksSubscriptionDirect(t *testing.T) {
	t.Run("direct-load-webhooks_subscription", func(t *testing.T) {
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-load-webhooks_subscription", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"2026_09_id01", "id01"} {
...
type webhooks_subscriptionDirectSetupResult struct {
...
func webhooks_subscriptionDirectSetup(mockres any) *webhooks_subscriptionDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"HUBSPOT_WEBHOOKS_TEST_WEBHOOKS_SUBSCRIPTION_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["HUBSPOT_WEBHOOKS_TEST_WEBHOOKS_SUBSCRIPTION_ENTID"]; ok {
		...
		return &webhooks_subscriptionDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &webhooks_subscriptionDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
