# HubspotWebhooks SDK test runner
...
class HubspotWebhooksTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            HubspotWebhooksTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = HubspotWebhooksTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = HubspotWebhooksTestRunner.getenv("HUBSPOT_WEBHOOKS_TEST_LIVE")
        override = HubspotWebhooksTestRunner.getenv("HUBSPOT_WEBHOOKS_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = HubspotWebhooksTestRunner.getenv(key)
        ...
        explain = HubspotWebhooksTestRunner.getenv("HUBSPOT_WEBHOOKS_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["HUBSPOT_WEBHOOKS_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if HubspotWebhooksTestRunner._test_control is not None:
            return HubspotWebhooksTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                HubspotWebhooksTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            HubspotWebhooksTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = HubspotWebhooksTestRunner.load_test_control()
...
def load_env_local():
    HubspotWebhooksTestRunner.load_env_local()


def env_override(m):
    return HubspotWebhooksTestRunner.env_override(m)


def entity_data(v):
    return HubspotWebhooksTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return HubspotWebhooksTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return HubspotWebhooksTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return HubspotWebhooksTestRunner.load_test_control()


def live_client_options():
    return HubspotWebhooksTestRunner.live_client_options()


def live_delay_ms():
    return HubspotWebhooksTestRunner.live_delay_ms()
