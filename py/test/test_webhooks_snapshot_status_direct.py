...
class TestWebhooksSnapshotStatusDirect:

    def test_should_direct_load_webhooks_snapshot_status(self):
        setup = _webhooks_snapshot_status_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-webhooks_snapshot_status", "live" if setup["live"] else "unit")
        if _skip:
...
def _webhooks_snapshot_status_direct_setup(mockres):
