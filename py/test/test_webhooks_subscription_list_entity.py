...
class TestWebhooksSubscriptionListEntity:

    def test_should_create_instance(self):
        testsdk = HubspotWebhooksSDK.test(None, None)
        ent = testsdk.WebhooksSubscriptionList(None)
    ...
    def test_should_stream(self):
        ...
        seen = list(base.WebhooksSubscriptionList(None).stream("list", None, None))
        assert len(seen) == 3
        ...
        if isinstance(cfg.get("feature"), dict) and "streaming" in cfg["feature"]:
            ...
            got = []
            for item in sdk.WebhooksSubscriptionList(None).stream("list", None, None):
    ...
    def test_should_run_basic_flow(self):
        setup = _webhooks_subscription_list_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["list"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "webhooks_subscription_list." + _op, "live" if _live else "unit")
            if _skip:
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set HUBSPOT_WEBHOOKS_TEST_WEBHOOKS_SUBSCRIPTION_LIST_ENTID JSON to run live")
        client = setup["client"]
        ...
        webhooks_subscription_list_ref01_data = None
        if len(webhooks_subscription_list_ref01_data_raw) > 0:
        ...
        assert isinstance(webhooks_subscription_list_ref01_list_result, list)



def _webhooks_subscription_list_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/webhooks_subscription_list/WebhooksSubscriptionListTestData.json")
    ...
    idmap_resolved = helpers.to_map(
        env.get("HUBSPOT_WEBHOOKS_TEST_WEBHOOKS_SUBSCRIPTION_LIST_ENTID"))
