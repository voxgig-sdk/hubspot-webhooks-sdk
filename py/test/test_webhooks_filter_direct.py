...
class TestWebhooksFilterDirect:

    def test_should_direct_load_webhooks_filter(self):
        setup = _webhooks_filter_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-webhooks_filter", "live" if setup["live"] else "unit")
        if _skip:
...
def _webhooks_filter_direct_setup(mockres):
