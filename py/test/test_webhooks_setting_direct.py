...
class TestWebhooksSettingDirect:

    def test_should_direct_load_webhooks_setting(self):
        setup = _webhooks_setting_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-webhooks_setting", "live" if setup["live"] else "unit")
        if _skip:
...
def _webhooks_setting_direct_setup(mockres):
