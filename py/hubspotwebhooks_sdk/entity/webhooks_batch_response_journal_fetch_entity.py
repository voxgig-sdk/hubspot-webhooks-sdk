...
class WebhooksBatchResponseJournalFetchEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return WebhooksBatchResponseJournalFetchEntity(self._client, opts)
    ...
    def data_get(self) -> WebhooksBatchResponseJournalFetch:
    ...
    def match_get(self) -> WebhooksBatchResponseJournalFetch:
    ...
    def load(self, reqmatch=None, ctrl=None) -> WebhooksBatchResponseJournalFetch:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.WebhooksBatchResponseJournalFetch().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
    ...
    def create(self, reqdata: WebhooksBatchResponseJournalFetchCreateData, ctrl=None) -> WebhooksBatchResponseJournalFetch:
        utility = self._utility
        ctx = utility.make_context({
            "opname": "create",
            "ctrl": ctrl,
