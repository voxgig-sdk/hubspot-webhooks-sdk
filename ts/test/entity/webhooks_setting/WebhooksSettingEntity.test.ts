...
describe('WebhooksSettingEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['update', 'load']) {
      if (!live && maybeSkipControl(t, 'entityOp', 'webhooks_setting.' + op, live)) return
    }
    ...
    let webhooks_setting_ref01_data = Object.values(setup.data.existing.webhooks_setting)[0] as any

    // UPDATE
    const webhooks_setting_ref01_ent = client.WebhooksSetting()
    const webhooks_setting_ref01_data_up0: any = {}

    const webhooks_setting_ref01_markdef_up0 = { name: 'targetUrl', value: 'Mark01-webhooks_setting_ref01_' + setup.now }
    ;(webhooks_setting_ref01_data_up0 as any)[webhooks_setting_ref01_markdef_up0.name] = webhooks_setting_ref01_markdef_up0.value
    ...
    assert((webhooks_setting_ref01_resdata_up0 as any)[webhooks_setting_ref01_markdef_up0.name] === webhooks_setting_ref01_markdef_up0.value)
