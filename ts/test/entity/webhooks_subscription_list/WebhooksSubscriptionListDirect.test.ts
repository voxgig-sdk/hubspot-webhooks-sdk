...
describe('WebhooksSubscriptionListDirect', async () => {
  ...
  test('direct-list-webhooks_subscription_list', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-webhooks_subscription_list', setup.live)) return
    if (skipIfMissingIds(t, setup, ["app01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_WEBHOOKS_TEST_WEBHOOKS_SUBSCRIPTION_LIST_ENTID']
