...
describe('WebhooksSubscriptionResponse1Direct', async () => {
  ...
  test('direct-load-webhooks_subscription_response_1', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-webhooks_subscription_response_1', setup.live)) return
    if (skipIfMissingIds(t, setup, ["subscription_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_WEBHOOKS_TEST_WEBHOOKS_SUBSCRIPTION_RESPONSE_1_ENTID']
