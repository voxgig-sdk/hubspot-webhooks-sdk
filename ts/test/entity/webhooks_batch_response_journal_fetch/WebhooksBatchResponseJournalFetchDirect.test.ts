...
describe('WebhooksBatchResponseJournalFetchDirect', async () => {
  ...
  test('direct-exists', async () => {
    const sdk = new HubspotWebhooksSDK({
  ...
  test('direct-load-webhooks_batch_response_journal_fetch', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-webhooks_batch_response_journal_fetch', setup.live)) return
    if (skipIfMissingIds(t, setup, ["batch_id01","count01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_WEBHOOKS_TEST_WEBHOOKS_BATCH_RESPONSE_JOURNAL_FETCH_ENTID']
