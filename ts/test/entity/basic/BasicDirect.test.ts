...
describe('BasicDirect', async () => {
  ...
  test('direct-load-basic', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup({ id: 'direct01' })
    if (maybeSkipControl(t, 'direct', 'direct-load-basic', setup.live)) return
    if (skipIfMissingIds(t, setup, ["offset_id01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_WEBHOOKS_TEST_BASIC_ENTID']
