...
describe('WebhooksCollectionResponseSubscriptionResponseNoPagingDirect', async () => {
  ...
  test('direct-list-webhooks_collection_response_subscription_response_no_paging', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-webhooks_collection_response_subscription_response_no_paging', setup.live)) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['HUBSPOT_WEBHOOKS_TEST_WEBHOOKS_COLLECTION_RESPONSE_SUBSCRIPTION_RESPONSE_NO_PAGING_ENTID']
