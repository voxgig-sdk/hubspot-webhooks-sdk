...
const __1 = require("..");
const SDK_NAME = 'HubspotWebhooksSDK';
...
function isRunnable(code) {
    return (/new\s+HubspotWebhooksSDK\b/.test(code) ||
        /\bHubspotWebhooksSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+HubspotWebhooksSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/HubspotWebhooksSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
