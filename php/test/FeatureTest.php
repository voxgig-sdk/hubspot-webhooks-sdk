...
// HubspotWebhooks SDK feature test
//
...
class FtCtrl extends HubspotWebhooksControl
{
...
class FtClient
{
    ...
    private HubspotWebhooksUtility $utility;

    public function __construct(HubspotWebhooksUtility $utility, array $options)
    {
    ...
    public function get_utility(): HubspotWebhooksUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (HubspotWebhooksContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public HubspotWebhooksContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (HubspotWebhooksContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = HubspotWebhooksConfig::make_feature($name);
    ...
    private static function buildUrl(HubspotWebhooksSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof HubspotWebhooksError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new HubspotWebhooksResponse($fetched);
        }
    ...
    private function fail(HubspotWebhooksContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (HubspotWebhooksContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
