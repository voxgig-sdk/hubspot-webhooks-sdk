...
class WebhooksBatchResponseSubscriptionEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = HubspotWebhooksSDK::test(null, null);
        $ent = $testsdk->WebhooksBatchResponseSubscription(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = webhooks_batch_response_subscription_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "webhooks_batch_response_subscription." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set HUBSPOT_WEBHOOKS_TEST_WEBHOOKS_BATCH_RESPONSE_SUBSCRIPTION_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $this->assertNotNull($webhooks_batch_response_subscription_ref01_data);
...
function webhooks_batch_response_subscription_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/webhooks_batch_response_subscription/WebhooksBatchResponseSubscriptionTestData.json';
    ...
    $entid_env_raw = getenv("HUBSPOT_WEBHOOKS_TEST_WEBHOOKS_BATCH_RESPONSE_SUBSCRIPTION_ENTID");
