...
class HubspotWebhooksMakeError
{
    public static function call(?HubspotWebhooksContext $ctx, mixed $err): mixed
    {
        if ($ctx === null) {
            require_once __DIR__ . '/../core/Context.php';
            $ctx = new HubspotWebhooksContext([], null);
        }
        $op = $ctx->op ?? new HubspotWebhooksOperation([]);
        $opname = $op->name;
        if ($opname === '' || $opname === '_') {
            $opname = 'unknown operation';
        }

        $result = $ctx->result ?? new HubspotWebhooksResult([]);
        $result->ok = false;
        ...
        $errmsg = ($err instanceof HubspotWebhooksError) ? $err->msg : (string)$err;
        ...
        $sdk_err = new HubspotWebhooksError('', $msg, $ctx);
        ...
        if ($err instanceof HubspotWebhooksError) {
