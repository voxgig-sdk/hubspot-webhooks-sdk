...
// HubspotWebhooks SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class HubspotWebhooksTestFeature extends HubspotWebhooksBaseFeature
{
    ...
    public function init(HubspotWebhooksContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (HubspotWebhooksContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = HubspotWebhooksParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (HubspotWebhooksContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(HubspotWebhooksContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = HubspotWebhooksParam::call($ctx, $k);
