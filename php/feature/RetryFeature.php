...
// HubspotWebhooks SDK retry feature
...
class HubspotWebhooksRetryFeature extends HubspotWebhooksBaseFeature
{
    ...
    public function init(HubspotWebhooksContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (HubspotWebhooksContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(HubspotWebhooksContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(HubspotWebhooksContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
