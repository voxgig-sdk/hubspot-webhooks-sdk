# hubspot-webhooks-cli

boru-driven command-line client **and** interactive REPL for the HubspotWebhooks
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/hubspot-webhooks-cli)
make build

# 2. See usage (words, entities, env vars)
./hubspot-webhooks-cli --help

# 3. Provide credentials once, via the environment
export HUBSPOT_WEBHOOKS_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./hubspot-webhooks-cli load 1 basic            # {id:1} shorthand
./hubspot-webhooks-cli load '{id:1}' basic       # explicit match map

# 5. Override the API base URL for a single call
HUBSPOT_WEBHOOKS_BASE=https://api.example.com ./hubspot-webhooks-cli load 1 basic

# 6. No arguments -> interactive REPL
./hubspot-webhooks-cli
hubspot-webhooks> load 1 basic
hubspot-webhooks> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/hubspot-webhooks-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/hubspot-webhooks-cli load 1 basic
   ```
...
```sh
./hubspot-webhooks-cli load 1 basic          # scalar shorthand for {id:1}
./hubspot-webhooks-cli load '{id:1}' basic     # explicit match map
```
...
```sh
export HUBSPOT_WEBHOOKS_APIKEY=sk_live_xxx            # API key
export HUBSPOT_WEBHOOKS_BASE=https://api.example.com  # optional: override the API base URL
./hubspot-webhooks-cli load 1 basic
```
...
Run with no arguments to open a REPL (prompt `hubspot-webhooks>`). Each line is
evaluated as its own boru expression:

```text
$ ./hubspot-webhooks-cli
hubspot-webhooks> load 1 basic
hubspot-webhooks> /help
hubspot-webhooks> /quit
```
...
| Variable | Purpose |
|----------|---------|
| `HUBSPOT_WEBHOOKS_APIKEY` | API key sent with every request. |
| `HUBSPOT_WEBHOOKS_BASE` | Optional override of the API base URL. |
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/hubspot-webhooks-cli`. |
...
(`./hubspot-webhooks-cli <expr>`) and interactively (the REPL), and expressions compose the
