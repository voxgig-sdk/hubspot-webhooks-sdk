...
//    only - a typed HubspotWebhooksContext there would make every ctx
...
final class HubspotWebhooksOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new HubspotWebhooksContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new HubspotWebhooksSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new HubspotWebhooksResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new HubspotWebhooksError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new HubspotWebhooksResponse($clean['response']);
    ...
    public static function err_from_map($m): ?HubspotWebhooksError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new HubspotWebhooksError(is_string($code) ? $code : '', $msg);
    }
