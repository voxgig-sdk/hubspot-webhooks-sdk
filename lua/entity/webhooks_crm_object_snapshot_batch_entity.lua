-- HubspotWebhooks SDK WebhooksCrmObjectSnapshotBatch entity

local vs = require("utility.struct.struct")
local helpers = require("core.helpers")

local WebhooksCrmObjectSnapshotBatchEntity = {}
WebhooksCrmObjectSnapshotBatchEntity.__index = WebhooksCrmObjectSnapshotBatchEntity


function WebhooksCrmObjectSnapshotBatchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WebhooksCrmObjectSnapshotBatchEntity)
  self._name = "webhooks_crm_object_snapshot_batch"
  self._client = client
...
function WebhooksCrmObjectSnapshotBatchEntity:get_name()
  return self._name
end


function WebhooksCrmObjectSnapshotBatchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WebhooksCrmObjectSnapshotBatchEntity.new(self._client, opts)
end
...
function WebhooksCrmObjectSnapshotBatchEntity:mark_deleted()
  self._deleted = true
end


function WebhooksCrmObjectSnapshotBatchEntity:deleted()
  return true == self._deleted
end


function WebhooksCrmObjectSnapshotBatchEntity:data_set(args)
  if args ~= nil then
...
function WebhooksCrmObjectSnapshotBatchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WebhooksCrmObjectSnapshotBatchEntity:match_set(args)
  if args ~= nil then
...
function WebhooksCrmObjectSnapshotBatchEntity:match_get()
...
function WebhooksCrmObjectSnapshotBatchEntity:stream(action, args, callopts)
...
---@param reqdata WebhooksCrmObjectSnapshotBatchCreateData
---@param ctrl? table
---@return WebhooksCrmObjectSnapshotBatch
---@return string? err
function WebhooksCrmObjectSnapshotBatchEntity:create(reqdata, ctrl)
  local utility = self._utility
...
function WebhooksCrmObjectSnapshotBatchEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return WebhooksCrmObjectSnapshotBatchEntity
