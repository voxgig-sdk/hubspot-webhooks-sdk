...
local WebhooksFilterEntity = {}
WebhooksFilterEntity.__index = WebhooksFilterEntity


function WebhooksFilterEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WebhooksFilterEntity)
  self._name = "webhooks_filter"
  self._client = client
...
function WebhooksFilterEntity:get_name()
  return self._name
end


function WebhooksFilterEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WebhooksFilterEntity.new(self._client, opts)
end
...
function WebhooksFilterEntity:mark_deleted()
  self._deleted = true
end


function WebhooksFilterEntity:deleted()
  return true == self._deleted
end


function WebhooksFilterEntity:data_set(args)
  if args ~= nil then
...
function WebhooksFilterEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WebhooksFilterEntity:match_set(args)
  if args ~= nil then
...
function WebhooksFilterEntity:match_get()
...
function WebhooksFilterEntity:stream(action, args, callopts)
...
---@param reqmatch WebhooksFilterLoadMatch
---@param ctrl? table
---@return WebhooksFilter
---@return string? err
function WebhooksFilterEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata WebhooksFilterCreateData
---@param ctrl? table
---@return WebhooksFilter
---@return string? err
function WebhooksFilterEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function WebhooksFilterEntity:_run_op(ctx, post_done)
  local utility = self._utility
