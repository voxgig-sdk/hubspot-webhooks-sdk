...
local WebhooksBatchResponseJournalFetchEntity = {}
WebhooksBatchResponseJournalFetchEntity.__index = WebhooksBatchResponseJournalFetchEntity


function WebhooksBatchResponseJournalFetchEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WebhooksBatchResponseJournalFetchEntity)
  self._name = "webhooks_batch_response_journal_fetch"
  self._client = client
...
function WebhooksBatchResponseJournalFetchEntity:get_name()
  return self._name
end


function WebhooksBatchResponseJournalFetchEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WebhooksBatchResponseJournalFetchEntity.new(self._client, opts)
end
...
function WebhooksBatchResponseJournalFetchEntity:mark_deleted()
  self._deleted = true
end


function WebhooksBatchResponseJournalFetchEntity:deleted()
  return true == self._deleted
end


function WebhooksBatchResponseJournalFetchEntity:data_set(args)
  if args ~= nil then
...
function WebhooksBatchResponseJournalFetchEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WebhooksBatchResponseJournalFetchEntity:match_set(args)
  if args ~= nil then
...
function WebhooksBatchResponseJournalFetchEntity:match_get()
...
function WebhooksBatchResponseJournalFetchEntity:stream(action, args, callopts)
...
---@param reqmatch WebhooksBatchResponseJournalFetchLoadMatch
---@param ctrl? table
---@return WebhooksBatchResponseJournalFetch
---@return string? err
function WebhooksBatchResponseJournalFetchEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
---@param reqdata WebhooksBatchResponseJournalFetchCreateData
---@param ctrl? table
---@return WebhooksBatchResponseJournalFetch
---@return string? err
function WebhooksBatchResponseJournalFetchEntity:create(reqdata, ctrl)
  local utility = self._utility
  local ctx = utility.make_context({
    opname = "create",
    ctrl = ctrl,
...
function WebhooksBatchResponseJournalFetchEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return WebhooksBatchResponseJournalFetchEntity
