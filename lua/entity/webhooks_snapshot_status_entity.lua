...
local WebhooksSnapshotStatusEntity = {}
WebhooksSnapshotStatusEntity.__index = WebhooksSnapshotStatusEntity


function WebhooksSnapshotStatusEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, WebhooksSnapshotStatusEntity)
  self._name = "webhooks_snapshot_status"
  self._client = client
...
function WebhooksSnapshotStatusEntity:get_name()
  return self._name
end


function WebhooksSnapshotStatusEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return WebhooksSnapshotStatusEntity.new(self._client, opts)
end
...
function WebhooksSnapshotStatusEntity:mark_deleted()
  self._deleted = true
end


function WebhooksSnapshotStatusEntity:deleted()
  return true == self._deleted
end


function WebhooksSnapshotStatusEntity:data_set(args)
  if args ~= nil then
...
function WebhooksSnapshotStatusEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function WebhooksSnapshotStatusEntity:match_set(args)
  if args ~= nil then
...
function WebhooksSnapshotStatusEntity:match_get()
...
function WebhooksSnapshotStatusEntity:stream(action, args, callopts)
...
---@param reqmatch WebhooksSnapshotStatusLoadMatch
---@param ctrl? table
---@return WebhooksSnapshotStatus
---@return string? err
function WebhooksSnapshotStatusEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function WebhooksSnapshotStatusEntity:_run_op(ctx, post_done)
  local utility = self._utility
...
return WebhooksSnapshotStatusEntity
